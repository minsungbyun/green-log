...
async function callGemini({ promptText, images = [], maxTokens = 600, jsonMode = false }) {
  ...
  // 일시적 오류(429/5xx/네트워크)면 자동 재시도: 총 3회 시도, 0.8s → 1.6s 대기
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      last = { ok: false, status: 0, detail: netErr.message || 'network error' };
    ...
    last = { ok: false, status: res.status, detail: detail.slice(0, 300) };
    if (!RETRYABLE.has(res.status)) break; // 재시도 불가 오류(400/401/403 등)는 즉시 중단
  }
  return last;
}
